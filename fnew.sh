...
# Logging disabled by request — output goes to terminal only, nothing written to disk.
...
echo "[*] Cleaning unnecessary log files..."
# Remove rotated/compressed logs (safe - these are already-archived old logs)
sudo find /var/log -type f \( -name "*.gz" -o -name "*.old" -o -name "*.1" -o -name "*.[0-9]" \) -delete 2>/dev/null || true
# Truncate (not delete) currently-open large logs so running services don't break
sudo find /var/log -type f -name "*.log" -size +50M -exec truncate -s 0 {} \; 2>/dev/null || true
# Remove our own self-heal log if a previous run left one behind
sudo rm -f /var/log/kali_self_heal.log 2>/dev/null || true
# Clear apt's historical term logs (safe, apt regenerates as needed)
sudo rm -f /var/log/apt/term.log.* /var/log/apt/history.log.* 2>/dev/null || true
# Clear old dpkg logs beyond the current one
sudo rm -f /var/log/dpkg.log.*.gz 2>/dev/null || true
...
echo "Log file saved to: (logging disabled - no file written)"
